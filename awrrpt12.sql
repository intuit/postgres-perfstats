...
select round((extract(epoch from (max(b.snap_time) - max(a.snap_time))) / 60)::numeric, 2) as "elapsed time (mins)",
       round((sum((b.total_time - coalesce(a.total_time, 0))) / 60000)::numeric, 2) as "db time (mins)"
  ...
  where a.instance_name = :'instance_name' and b.snap_id = :esnap;


select round((sum((b.total_time - coalesce(a.total_time, 0))))::numeric, 2) as total_time
  from perfstat.pg_stat_statements_hist a right join perfstat.pg_stat_statements_hist b
    on (a.instance_name = b.instance_name and a.userid = b.userid and a.dbid = b.dbid and a.queryid = b.queryid and a.query = b.query and a.snap_id = :bsnap)
  where a.instance_name = :'instance_name' and b.snap_id = :esnap \gset
...
select round(extract(epoch from (b.snap_time - a.snap_time))::numeric, 2) as elapsed_sec,
  ...
  from perfstat.pg_stat_database_hist a right join perfstat.pg_stat_database_hist b
    on (a.instance_name = b.instance_name and a.datid = b.datid and a.snap_id = :bsnap)
  where a.instance_name = :'instance_name' and b.snap_id = :esnap \gset
...
select 'calls' as metric, 
  ...
  from perfstat.pg_stat_statements_hist a right join perfstat.pg_stat_statements_hist b
    on (a.instance_name = b.instance_name and a.dbid = b.dbid and a.userid = b.userid and a.queryid = b.queryid and a.query = b.query and a.snap_id = :bsnap)
  where a.instance_name = :'instance_name' and b.snap_id = :esnap  
union
select 'db_time' as metric, 
       round((sum(b.total_time - coalesce(a.total_time, 0))::numeric) / :elapsed_sec::numeric, 2) "per second",
       round((sum(b.total_time - coalesce(a.total_time, 0))::numeric) / :total_xact::numeric, 2) "per transaction"
  from perfstat.pg_stat_statements_hist a right join perfstat.pg_stat_statements_hist b
    on (a.instance_name = b.instance_name and a.dbid = b.dbid and a.userid = b.userid and a.queryid = b.queryid and a.query = b.query and a.snap_id = :bsnap)
  where a.instance_name = :'instance_name' and b.snap_id = :esnap  
order by 1;  
...
select 
       round((b.total_time - coalesce(a.total_time, 2))::numeric, 2) "total time",
       ...
       round(((b.total_time - coalesce(a.total_time, 2))::numeric / :total_time) * 100, 2) "%total",
       round(((b.total_time - coalesce(a.total_time, 0))::numeric / case (b.calls - coalesce(a.calls, 0)) when 0 then 1 else (b.calls - coalesce(a.calls, 0)) end)::numeric, 2) "avg time",
  ...
  and b.total_time - coalesce(a.total_time, 2) > 0
  and a.instance_name = :'instance_name'
  order by b.total_time - coalesce(a.total_time, 0) desc
  limit 30;
...
select b.schemaname "schema",
  ...
  from perfstat.pg_stat_all_tables_hist a right join perfstat.pg_stat_all_tables_hist b
    on (a.instance_name = b.instance_name and a.schemaname = b.schemaname and a.relname = b.relname and a.snap_id = :bsnap)
  where b.snap_id = :esnap
